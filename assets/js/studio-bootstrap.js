...
(function bootstrapStudioChart(){
  ...
  async function boot(){
    ...
    (function wirePreClosePanel(){
      if(!DC.PreclosePanel || typeof DC.PreclosePanel.mount !== 'function' || !preCloseBtn) return;
      var precloseInstance = DC.PreclosePanel.mount({
        getCandles: function(symbol){
          return window.DannyChart.FyersService.getCandles({ symbol: symbol, timeframe: '15m', limit: 180 });
        }
      });
      preCloseBtn.addEventListener('click', function(){
        ...
        var symbol = (s && s.symbol) || 'NIFTY';
        var MarketSession = window.DannyChart && window.DannyChart.MarketSession;
        if(!MarketSession || !MarketSession.isIndex(symbol)) return; // muted button — not applicable, no-op rather than opening an empty/wrong panel
